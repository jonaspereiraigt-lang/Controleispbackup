            ...
            <div className="space-y-4">
              ...
              {activeTab === 'financeiro' && (
                <div className="space-y-6">
                  ...
                  {selectedProvider && !selectedProvider.financial_generated && (
                    <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-6 mb-6">
                      <div className="flex items-start gap-4">
                        <div className="text-yellow-600 text-3xl">⚠️</div>
                        <div className="flex-1">
                          <h3 className="font-bold text-yellow-900 text-lg mb-2">
                            Financeiro Pendente - Provedor Aguardando Liberação
                          </h3>
                          <p className="text-yellow-800 mb-4">
                            Este provedor ainda não teve o financeiro gerado. Ele pode fazer login mas 
                            não tem acesso às funcionalidades até que você gere o boleto ou PIX de pagamento.
                          </p>
                          <div className="flex gap-3">
                            <button
                              onClick={async () => {
                                if (window.confirm('Gerar BOLETO para este provedor?')) {
                                  try {
                                    const token = localStorage.getItem('admin_token');
                                    await axios.post(
                                      `${API}/admin/providers/${selectedProvider.id}/generate-financial`,
                                      { type: 'boleto', amount: formData.plan_value },
                                      { headers: { Authorization: `Bearer ${token}` } }
                                    );
                                    alert('Boleto gerado! Provedor liberado para acesso.');
                                    loadProviders();
                                    loadProviderPayments(selectedProvider.id);
                                  } catch (error) {
                            ...
                            <button
                              onClick={async () => {
                                if (window.confirm('Gerar PIX para este provedor?')) {
                                  try {
                                    const token = localStorage.getItem('admin_token');
                                    await axios.post(
                                      `${API}/admin/providers/${selectedProvider.id}/generate-financial`,
                                      { type: 'pix', amount: formData.plan_value },
                                      { headers: { Authorization: `Bearer ${token}` } }
                                    );
                                    alert('PIX gerado! Provedor liberado para acesso.');
                                    loadProviders();
                                    loadProviderPayments(selectedProvider.id);
                                  } catch (error) {
